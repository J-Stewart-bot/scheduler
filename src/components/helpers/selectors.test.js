...
test("getInterviewersForDay returns an array", () => {
  const result = helpers.default.getInterviewersForDay(state, "Monday");
  expect(Array.isArray(result)).toBe(true);
});
...
test("getInterviewersForDay returns an array containing the correct appointment objects", () => {
  const [first, second] = helpers.default.getInterviewersForDay(state, "Tuesday");
  expect(first).toEqual(state.interviewers["1"]);
  expect(second).toEqual(state.interviewers["2"]);
});

test("getInterviewersForDay returns an empty array when the days data is empty", () => {
  const result = helpers.default.getInterviewersForDay({ days: [] }, "Monday");
  expect(result.length).toEqual(0);
});

test("getInterviewersForDay returns an empty array when the day is not found", () => {
  const result = helpers.default.getInterviewersForDay(state, "Wednesday");
  expect(result.length).toEqual(0);
});